      ...
      <div class="s_section">
          ...
          <div class="s_section__wrap" :class="{'loader': loader, 'opacity': popup.state}">
              ...
              <div class="s_section__body">
                  ...
                  <table-in @emitPopup="statePopup" @setItemData="setItemData" :leads="leads" :height="height" :loaderTable="loaderTable"/>
                  <pagination-in v-show="!loaderTable && leads !== null" :pagination="pagination" @emitPage="getParameter" @page="addPage"/>
              </div>
          </div>
          <popup-in v-if="popup.state" :class="{'visible':popup.animate}" :item="item"  @emitPopup="statePopup" />
      </div>
</template>
...
import popup_in from './../popup/popup';
  ...
  methods: {
    ...
    getHeight() {
        if(window.innerWidth < 998){
            this.parameters.limit = 'limit=10';
    ...
    getLoadLead() {
     ...
     getLead() {
        ...
        this.$axios.get(g.baseUrl + g.lead + this.parameter).then(result=>{
            this.url = result.data.response.leads
            this.success = result.data.success;
            if (result.data.success) {
              if (result.data.response.leads.length !== 0) {
                this.leads = result.data.response.leads;
              } else {
                this.leads = null;
              }
              this.pagination = result.data.response.pagination;
            } else {
    ...
    getParameter(value, name) {
      ...
      if (name === 'filters') {
        ...
        this.parameters['providers'] = value.provider;
      } else {
